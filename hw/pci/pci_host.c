...
void pci_data_write(PCIBus *s, uint32_t addr, uint32_t val, unsigned len)
{
    ...
    if (switch_pci_cfg_log_enabled(&cfg_write_log_count)) {
        fprintf(stderr,
                "Switch: pci cfg write bus=%u dev=%u fn=%u reg=0x%02x len=%u val=0x%08x dev=%s\n",
                extract32(addr, 16, 8),
                extract32(addr, 11, 5),
                extract32(addr, 8, 3),
                config_addr, len, val,
...
uint32_t pci_data_read(PCIBus *s, uint32_t addr, unsigned len)
{
    ...
    if (switch_pci_cfg_log_enabled(&cfg_read_log_count)) {
        fprintf(stderr,
                "Switch: pci cfg read  bus=%u dev=%u fn=%u reg=0x%02x len=%u val=0x%08x dev=%s\n",
                extract32(addr, 16, 8),
                extract32(addr, 11, 5),
                extract32(addr, 8, 3),
                config_addr, len, ret, pci_dev->name);
    }
    cfg_read_log_count++;
    return ret;
}
